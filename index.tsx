...
function checkEnableDecryptButton() { decryptButton.disabled = !(decryptIsPrivateKeyLoaded && decryptArePairsSelected); }

function updateStatus(areaId: 'encrypt' | 'decrypt', message: string | null = null, options: { showSpinner?: boolean, isError?: boolean, showWarning?: boolean } = {}) {
    const { showSpinner = false, isError = false, showWarning = false } = options;
    const statusArea = document.getElementById(`${areaId}-status-area`); const statusMessage = document.getElementById(`${areaId}-status-message`); const errorMessage = document.getElementById(`${areaId}-error-message`); const warningMessage = document.getElementById(`${areaId}-warning-message`); const spinner = document.getElementById(`${areaId}-spinner`); const resultArea = document.getElementById(`${areaId}-result-area`);
    if (!statusArea || !statusMessage || !errorMessage || !spinner || !resultArea) return;
    statusArea.style.display = message || showSpinner ? 'block' : 'none'; statusMessage.textContent = message && !isError ? message : ''; statusMessage.style.display = message && !isError ? 'block' : 'none';
    errorMessage.textContent = message && isError ? `Error: ${message}` : ''; errorMessage.style.display = message && isError ? 'block' : 'none';
    spinner.style.display = showSpinner ? 'block' : 'none'; if (warningMessage) warningMessage.style.display = showWarning ? 'block' : 'none';
...
function showResult(areaId: 'encrypt' | 'decrypt', data: { downloadUrl: string, zipName?: string }) {
     updateStatus(areaId, null); const resultArea = document.getElementById(`${areaId}-result-area`); const downloadLink = document.getElementById(`${areaId}-download-link`); const currentForm = areaId === 'encrypt' ? encryptionForm : decryptionForm; const currentButton = areaId === 'encrypt' ? encryptButton : decryptButton;
     if (resultArea && downloadLink && currentButton && currentForm) { resultArea.style.display = 'block'; (downloadLink as HTMLAnchorElement).href = data.downloadUrl; (downloadLink as HTMLAnchorElement).download = data.zipName || `${areaId}ed_files.zip`; currentButton.disabled = true; currentForm.style.display = 'none'; }
}

function showError(areaId: 'encrypt' | 'decrypt', message: string | null) {
    const button = areaId === 'encrypt' ? encryptButton : decryptButton;
    if (message === null) { updateStatus(areaId, null); } // Clear messages only
    else { updateStatus(areaId, message, { isError: true }); button.disabled = false; } // Ensure button is enabled on error display for potential re-try (checkEnable will verify pre-reqs)
    // Always re-check actual pre-requisites after clearing or showing error
    if (areaId === 'encrypt') checkEnableEncryptButton(); else checkEnableDecryptButton();
    if (message) console.error(`[${areaId}] Error:`, message);
}
...
// --- Decryption Process (No changes needed here) ---
...
// Encryption Listeners (No changes)
...
decryptPrivateKeyInput.addEventListener('change', async (event) => {
     ...
     decryptIsPrivateKeyLoaded = false; decryptPrivateKeyStatus.style.display = 'none';
    if (input.files && input.files.length > 0) {
        const file = input.files[0]; try { decryptPrivateKeyPem = await file.text(); if (!decryptPrivateKeyPem.includes('-----BEGIN') || !decryptPrivateKeyPem.includes('PRIVATE KEY-----')) throw new Error("Formato de llave privada inválido."); decryptIsPrivateKeyLoaded = true; decryptPrivateKeyStatus.style.display = 'block'; showError('decrypt', null); } catch (error: any) { showError('decrypt', `Error al leer llave privada: ${error.message}`); decryptPrivateKeyPem = null; }
    }
    checkEnableDecryptButton(); // Check button state after processing
});

selectDecryptFolderButton.addEventListener('click', async () => {
    // Reset state for this operation
    decryptArePairsSelected = false; decryptSelectedPairs = [];
    decryptFolderStatus.style.display = 'none'; decryptFileListElement.innerHTML = ''; decryptFileCountError.style.display = 'none';
    updateStatus('decrypt',"Buscando pares .enc/.key...", {showSpinner: true});
    ...
    try {
        ...
        for await (const entry of dirHandle.values()) {
            if (entry.kind === 'file') {
                 // @ts-ignore - Experimental API
                const file = await entry.getFile(); const name = file.name; processedFileCount++;
                ...
                const existing = fileMap.get(baseName) || {}; if (isEnc) existing.encFile = file; if (isKey) existing.keyFile = file; fileMap.set(baseName, existing);
            }
        }

        // Identify valid pairs and update UI list
        for (const [baseName, files] of fileMap.entries()) {
            if (files.encFile && files.keyFile) {
                if (decryptSelectedPairs.length < 20) {
                    ...
                    const listItem = document.createElement('li'); listItem.textContent = `${baseName} (.enc + .key)`; decryptFileListElement.appendChild(listItem);
                } else { decryptFileCountError.style.display = 'block'; updateStatus('decrypt', 'Se encontraron más de 20 pares, solo se procesarán los primeros 20.'); break; }
            }
        }

        // Update status based on findings
        if (decryptSelectedPairs.length > 0) {
            decryptArePairsSelected = true; // Set the flag indicating pairs are ready
            ...
            decryptFolderStatus.style.display = 'block';
             // Clear status message/spinner as the search is complete
            updateStatus('decrypt', null);
            showError('decrypt', null); // Clear previous errors
        } else {
             decryptArePairsSelected = false; // Ensure flag is false
        ...
        // Explicitly check button state after all processing (success or error)
        checkEnableDecryptButton();
         // Final cleanup of spinner if no message/error is displayed
         ...
         if (!statusMsg && !errorMsg && !decryptArePairsSelected && decryptSelectedPairs.length === 0) { // Also check if pairs were found
              updateStatus('decrypt', null);
         } else if (!statusMsg && !errorMsg && decryptArePairsSelected){
              // If pairs were found, but no other message, still hide spinner etc.
              updateStatus('decrypt', null);
         }
    }
});
...
decryptionForm.addEventListener('submit', (event) => { event.preventDefault(); if (decryptButton.disabled) return; if (!decryptIsPrivateKeyLoaded || !decryptArePairsSelected) { showError("decrypt", "Selecciona la llave privada y una carpeta con pares .enc/.key válidos."); return; } decryptButton.disabled = true; performDecryption(); });
decryptResetButton.addEventListener('click', resetDecryptUI);

// --- Initial State ---
encryptTabButton.classList.add('active'); encryptTabContent.classList.add('active'); decryptTabButton.classList.remove('active'); decryptTabContent.classList.remove('active'); resetEncryptUI(); resetDecryptUI();
